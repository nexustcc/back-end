 ...
 */
 ...
 router.delete("/deletarProfessor/:idProfessor", (req, res) => {
     mysql.connect((error, connection) => {
         ...
         const sqlPastaCompartilhada = 'DELETE FROM tblPastaCompartilhada WHERE idProfessor = ?'
         connection.query(sqlPastaCompartilhada, idProfessor, (error, result) => {
             if (error) { return res.status(500).send({ error: error, response: null }) }
 
             const sqlProfessorGrupo = 'DELETE FROM tblProfessorGrupo WHERE idProfessor = ?'
             connection.query(sqlProfessorGrupo, idProfessor, (error, result) => {
                 if (error) { return res.status(500).send({ error: error, response: null }) }
 
                 const sqlTurmaProfessor = 'DELETE FROM tblTurmaProfessor WHERE idProfessor = ?'
                 connection.query(sqlTurmaProfessor, idProfessor, (error, result) => {
                     if (error) { return res.status(500).send({ error: error, response: null }) }
     
                     const sqlProfessorCurso = 'DELETE FROM tblProfessorCurso WHERE idProfessor = ?'
                     connection.query(sqlProfessorCurso, idProfessor, (error, result) => {
                         ...
                         connection.query(sqlAutor, idProfessor, (error, result) => {
                             ...
                             const sqlAtividade = 'DELETE FROM tblAtividade WHERE idProfessor = ?'
                             connection.query(sqlAtividade, idProfessor, (error, result) => {
                                 if (error) { return res.status(500).send({ error: error, response: null }) }
 
                                 const sqlIdUsuario = 'SELECT tblUsuario.idUsuario FROM tblUsuario INNER JOIN tblProfessor ON tblProfessor.idUsuario = tblUsuario.idUsuario WHERE tblProfessor.idProfessor = ?'
                                 connection.query(sqlIdUsuario, idProfessor, (error, result) => {
                                     ...
                                     const sqlProfessor = 'DELETE FROM tblProfessor WHERE idProfessor = ?'
                                     connection.query(sqlProfessor, idProfessor, (error, result) => {
                                     if (error) { return res.status(500).send({ error: error, response: null }) }
     ...
     mysql.connect((error, connection) => {
         ...
         connection.query(
             ...
             (error, result, field) => {
                 ...
                 const sqlAvaliadores = "SELECT DISTINCT tblAvaliador.idAvaliador, tblUsuario.nome FROM tblUsuario INNER JOIN tblAvaliador ON tblAvaliador.idUsuario = tblUsuario.idUsuario INNER JOIN tblAvaliadorGrupo ON tblAvaliadorGrupo.idAvaliador = tblAvaliador.idAvaliador INNER JOIN tblgrupo ON tblgrupo.idGrupo = tblAvaliadorGrupo.idGrupo INNER JOIN tblProfessorGrupo ON tblProfessorGrupo.idGrupo = tblGrupo.idGrupo WHERE tblProfessorGrupo.idProfessor = ? ORDER BY tblUsuario.nome";
                 connection.query(
                     sqlAvaliadores,
                     req.params.idProfessor,
                     (error, result, field) => {
